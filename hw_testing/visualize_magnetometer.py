"""
Basic Matplotlib Live Plotter for TLV493D Magnetometer Data
...
def main():
    ...
    parser.add_argument(
        "-b", "--baud", type=int, default=DEFAULT_BAUD, help="Baud rate"
    )
    parser.add_argument(
        "-f", "--force", action="store_true", help="Force open the port even if busy"
    )
    ...
    try:
        ...
        ani = FuncAnimation(
            fig,
            update_plot,
            fargs=(serial_connection, fig, axes),
            interval=50,
            save_count=100,
        )  # 50ms interval, 100 frames cache
